...
function resetSceneWorkspace() {
  ...
  const workspaceWrap = $('sceneWorkspaceWrap');
  if (workspaceWrap) workspaceWrap.style.display = 'none';
  const imageCard = $('singleImageCard');
  const videoCard = $('singleVideoCard');
  if (imageCard) imageCard.style.display = '';
  if (videoCard) videoCard.style.display = '';
}

function normalizeTextBlock(v = '') {
  return String(v || '').replace(/^```[\w-]*\n?/, '').replace(/```$/,'').trim();
}

function parseSceneBlocks(text = '', kind = 'image') {
  const src = normalizeTextBlock(text);
  if (!src) return [];
  const kindPattern = kind === 'image'
    ? '(?:IMAGE(?:\s*PROMPT)?)'
    : '(?:VIDEO(?:\s*\+\s*AUDIO)?\s*PROMPT|VDO(?:\s*\+\s*AUDIO)?\s*PROMPT|VIDEO|VDO)';
  const headerRegex = new RegExp(`SCENE[_\s]*(\d+)[_\s]*${kindPattern}\s*:`, 'gi');
  const matches = [...src.matchAll(headerRegex)];
  if (!matches.length) return [];
  return matches.map((match, index) => {
    const sceneNo = Number(match[1] || 0);
    const start = match.index + match[0].length;
    const end = index + 1 < matches.length ? matches[index + 1].index : src.length;
    return { sceneNo, content: src.slice(start, end).trim() };
  }).filter(block => block.sceneNo && block.content);
}

function parseScenePrompts(imagePrompt = '', videoPrompt = '', sceneCount = 1) {
  const imageBlocks = parseSceneBlocks(imagePrompt, 'image');
  const videoBlocks = parseSceneBlocks(videoPrompt, 'video');
  const maxScenes = Number(sceneCount || 1);
  const byNo = new Map();
  for (let i = 1; i <= maxScenes; i++) byNo.set(i, { sceneNo: i, imagePrompt: '', videoPrompt: '' });
  imageBlocks.forEach(block => { if (!byNo.has(block.sceneNo)) byNo.set(block.sceneNo, { sceneNo: block.sceneNo, imagePrompt: '', videoPrompt: '' }); byNo.get(block.sceneNo).imagePrompt = block.content; });
  videoBlocks.forEach(block => { if (!byNo.has(block.sceneNo)) byNo.set(block.sceneNo, { sceneNo: block.sceneNo, imagePrompt: '', videoPrompt: '' }); byNo.get(block.sceneNo).videoPrompt = block.content; });
  return [...byNo.values()].sort((a,b)=>a.sceneNo-b.sceneNo).filter(s => s.imagePrompt || s.videoPrompt);
}

function buildCombinedScenePrompt(type = 'image') {
  const wrap = getSceneWorkspace();
  if (!wrap) return '';
  const selector = type === 'image' ? '.scene-image-textarea' : '.scene-video-textarea';
  const suffix = type === 'image' ? 'IMAGE' : 'VIDEO';
  return [...wrap.querySelectorAll('.scene-card')].map(card => {
    const sceneNo = Number(card.dataset.sceneNo || 0);
    const textarea = card.querySelector(selector);
    const value = (textarea?.value || '').trim();
    if (!sceneNo || !value) return '';
    return `SCENE_${sceneNo}_${suffix}_PROMPT:
${value}`;
  }).filter(Boolean).join('\n\n');
}
...
function toggleScenePromptEdit(sceneNo, type) {
  ...
  const enteringEdit = textarea.readOnly;
  textarea.readOnly = !enteringEdit;
  textarea.classList.toggle('editing', enteringEdit);
  if (saveBtn) saveBtn.style.display = enteringEdit ? 'inline-flex' : 'none';
  if (editBtn) editBtn.textContent = enteringEdit ? 'ยกเลิก' : 'แก้ไข';
  if (enteringEdit) textarea.focus();
}
...
async function saveScenePromptEdit(sceneNo, type) {
  ...
  if (type === 'image') $('imagePrompt').value = buildCombinedScenePrompt('image');
  else $('videoPrompt').value = buildCombinedScenePrompt('video');
  try {
    if (currentUser && currentHistoryId) {
      await updateDoc(doc(db, 'promptHistory', currentHistoryId), type === 'image' ? { imagePrompt: $('imagePrompt').value, updatedAt: serverTimestamp() } : { videoPrompt: $('videoPrompt').value, updatedAt: serverTimestamp() });
      await renderHistory();
    }
...
function renderSceneWorkspace(sceneCount, imagePrompt = '', videoPrompt = '') {
  ...
  $('sceneWorkspaceWrap').style.display = 'block';
  $('singleImageCard').style.display = 'none';
      ...
      <div class="scene-grid">
        <section class="prompt-card scene-subcard">
          <div class="prompt-top">
            <h3>IMAGE PROMPT</h3>
            <div class="row" style="align-items:center;justify-content:flex-end;gap:8px;flex:0 0 auto">
              <span class="tag image">Image</span>
              <div class="edit-actions">
                <button class="btn btn-outline edit-scene-image-btn" data-scene="${scene.sceneNo}" style="padding:10px 12px">แก้ไข</button>
                <button class="btn btn-green save-scene-image-btn" data-scene="${scene.sceneNo}" style="padding:10px 12px;display:none">บันทึก</button>
                <button class="btn btn-dark copy-scene-image-btn" data-scene="${scene.sceneNo}" style="padding:10px 12px">คัดลอก</button>
              </div>
            </div>
          </div>
        ...
        <section class="prompt-card scene-subcard">
          <div class="prompt-top">
            <h3>VIDEO + AUDIO PROMPT</h3>
            <div class="row" style="align-items:center;justify-content:flex-end;gap:8px;flex:0 0 auto">
              <span class="tag video">Video</span>
              <div class="edit-actions">
                <button class="btn btn-outline edit-scene-video-btn" data-scene="${scene.sceneNo}" style="padding:10px 12px">แก้ไข</button>
                <button class="btn btn-green save-scene-video-btn" data-scene="${scene.sceneNo}" style="padding:10px 12px;display:none">บันทึก</button>
                <button class="btn btn-dark copy-scene-video-btn" data-scene="${scene.sceneNo}" style="padding:10px 12px">คัดลอก</button>
              </div>
            </div>
          </div>
...
- If scene count is greater than 1, split both prompts using exact headers only:
...
- If scene count is greater than 1, return image_prompt and video_prompt split into clear blocks using these exact headers only: SCENE_1_IMAGE_PROMPT:, SCENE_2_IMAGE_PROMPT:, ... and SCENE_1_VIDEO_PROMPT:, SCENE_2_VIDEO_PROMPT:, ...
- Final only.`;
}
...
async function generatePrompts() {
  ...
  try {
    ...
    renderSceneWorkspace(data.sceneCount, result.image_prompt, result.video_prompt);
