...
async function saveScenePromptEdit(sceneNo, type){
  ...
  const copyBtn = card.querySelector(type === 'image' ? '.copy-scene-image-btn' : '.copy-scene-video-btn');
  clearCopyButtonState(copyBtn);
  try{
...
function markCopyButtonCopied(btn){
  if(!btn) return;
  btn.dataset.copied = '1';
  btn.classList.add('btn-copied');
  const original = btn.dataset.originalText || btn.textContent || 'คัดลอก';
  btn.dataset.originalText = original;
  btn.innerHTML = '✓ คัดลอกแล้ว';
}

function clearCopyButtonState(btn){
  if(!btn) return;
...
async function copyBlock(id,btn){ const text=$(id)?.value||''; if(!text.trim()) return showToast('ยังไม่มีข้อความให้คัดลอก'); await navigator.clipboard.writeText(text); markCopyButtonCopied(btn); }
