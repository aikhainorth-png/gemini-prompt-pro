...
function getFormData(){ return { product:$('product')?.value.trim()||'', location:$('location')?.value.trim()||'', view:$('view')?.value.trim()||'', promptStrategy:$('promptStrategy')?.value||localStorage.getItem(LS_PROMPT_STRATEGY)||'viral', gemMode:$('gemMode')?.value||'signboard', providerMode:$('providerMode')?.value||'gemini', voiceType:$('voiceType')?.value||'ผู้หญิง', viralTone:$('viralTone')?.value||'ล้างสต๊อก', sceneCount:Number($('sceneCount')?.value||1), duration:Number($('duration')?.value||10), textOverlayEnabled: !!$('textOverlayEnabled')?.checked, textOverlayStyle:$('textOverlayStyle')?.value||'', textOverlayScene:$('textOverlayScene')?.value||'all', textOverlayHook:$('textOverlayHook')?.value.trim()||'', textOverlayPosition:$('textOverlayPosition')?.value||'center', textOverlaySize:$('textOverlaySize')?.value||'medium', h2OverlayEnabled: !!$('h2OverlayEnabled')?.checked, h2OverlayStyle:$('h2OverlayStyle')?.value||'', h2OverlayScene:$('h2OverlayScene')?.value||'all', h2OverlayHook:$('h2OverlayHook')?.value.trim()||'' }; }
...
function updateSummary(){ const d=getFormData(); if($('summaryPreview')) $('summaryPreview').textContent=[`สินค้า: ${d.product||'-'}`,`สถานที่: ${d.location||'-'}`,`มุมมองสินค้า: ${d.view||'-'}`,`GEM MODE: ${getModeSource().getGemModeConfig(d.gemMode).label}`,`AI Provider: ${d.providerMode||'gemini'}`,`ตัวละคร + เสียงพากย์: ${d.voiceType||'-'}`,`โทนไวรัล: ${d.viralTone||'-'}`,`จำนวน Scene: ${d.sceneCount}`,`เวลาทั้งหมด: ${d.duration} วินาที`,`เวลาเฉลี่ยต่อ Scene: ${formatPerScene(d.duration,d.sceneCount)}`,`TEXT OVERLAY: ${d.textOverlayEnabled ? (d.textOverlayStyle || 'auto') : 'ปิด'}`,`H2: ${d.h2OverlayEnabled ? (d.h2OverlayStyle || 'auto') : 'ปิด'}`,`STORE VIRAL PACK V3: 200 TEXT / 40 H2`].join('\n'); if($('statScene')) $('statScene').textContent=String(d.sceneCount); if($('statDuration')) $('statDuration').textContent=`${d.duration}s`; if($('statPerScene')) $('statPerScene').textContent=formatPerScene(d.duration,d.sceneCount); }
...
function loadExample(slot){
  ...
  const exampleDefaults = {
    voiceType: ['แม่และเด็ก','ชุดชั้นใน','เครื่องสำอาง','ครีมบำรุงผิว','แฟชั่น'].some(word => mode.label.includes(word)) ? 'ผู้หญิง' : (($('voiceType')?.value) || 'ผู้หญิง'),
...
function getCharacterVoiceSpec(voiceType='ผู้หญิง'){
  const vt = String(voiceType || 'ผู้หญิง');
  const map = {
    'ผู้หญิง': {
      image: 'Main character: realistic Thai / Asian young woman, photorealistic human, natural beauty, natural black or dark brown hair, warm friendly expression, live-action commercial presenter.',
      voice: 'Voice: Thai female natural commercial voice, warm, clear, friendly, persuasive.',
      short: 'Thai female presenter'
    },
    'ผู้ชาย': {
      image: 'Main character: realistic Thai / Asian young man, photorealistic human, natural masculine facial features, neat dark hair, confident friendly expression, live-action commercial presenter.',
      voice: 'Voice: Thai male natural commercial voice, confident, clear, trustworthy, persuasive.',
      short: 'Thai male presenter'
    },
    'หญิงชรา': {
      image: 'Main character: realistic Thai / Asian elderly woman, photorealistic human, natural aged facial features, gentle wrinkles, soft grey or dark hair, warm trustworthy expression.',
      voice: 'Voice: Thai elderly female voice, calm, warm, wise, trustworthy.',
      short: 'Thai elderly female presenter'
    },
    'ชายชรา': {
      image: 'Main character: realistic Thai / Asian elderly man, photorealistic human, natural aged facial features, gentle wrinkles, grey or dark hair, calm trustworthy expression.',
      voice: 'Voice: Thai elderly male voice, calm, deep, wise, trustworthy.',
      short: 'Thai elderly male presenter'
    },
    'เด็กผู้หญิง': {
      image: 'Main character: realistic Thai / Asian girl, photorealistic human child, natural child proportions, bright cheerful expression, neat dark hair.',
      voice: 'Voice: Thai girl voice, cheerful, cute, energetic, natural.',
      short: 'Thai girl presenter'
    },
    'เด็กผู้ชาย': {
      image: 'Main character: realistic Thai / Asian boy, photorealistic human child, natural child proportions, playful bright expression, neat dark hair.',
      voice: 'Voice: Thai boy voice, playful, bright, energetic, natural.',
      short: 'Thai boy presenter'
    }
  };
  return map[vt] || map['ผู้หญิง'];
}

function ensureCharacterBlocksInScenePrompts(imagePrompt='', videoPrompt='', character=null, sceneCount=1){
  if(!character?.enabled || Number(sceneCount||1) <= 1) return { imagePrompt, videoPrompt };
  const scenes = parseScenePrompts(imagePrompt, videoPrompt, sceneCount);
  if(!scenes.length) return { imagePrompt, videoPrompt };
  const injectImage = scenes.map(scene => {
    const block = [
...
${character.dnaBlock}`,
      `Continuity Lock:
${character.lockBlock}`,
      scene.imagePrompt || synthesizeImagePromptFromVideo(scene.videoPrompt || '', scene.sceneNo)
    ].filter(Boolean).join('
...
');
  const injectVideo = scenes.map(scene => {
    const block = [
      `Character ID: ${character.characterId}`,
      `Character DNA Block:
${character.dnaBlock}`,
...
function buildSystemInstruction(d = getPreparedFormData(getFormData())){
  ...
  const voiceSpec = getCharacterVoiceSpec(d.voiceType);
...
- Selected character + voice profile: ${voiceSpec.short}
- ${voiceSpec.image}
- ${voiceSpec.voice}
- The selected voice type controls narrator gender, age, and character profile.
...
Character + voice type: ${d.voiceType}
...
- Character appearance must follow the selected character + voice type: ${d.voiceType}.
- ${voiceSpec.image}
- ${voiceSpec.voice}
- Narration gender, age, and performance must follow the selected character + voice type: ${d.voiceType}.
...
- If scene count is greater than 1 and CHARACTER FACTORY PRO MAX is active, every single scene block must explicitly contain Character ID, Character DNA Block, and Continuity Lock before the scene prompt body so the same character appears in every scene.
...
async function useHistoryItem(id){
  try{
    ...
    if($('voiceType')) $('voiceType').value=item.voiceType||'ผู้หญิง';
